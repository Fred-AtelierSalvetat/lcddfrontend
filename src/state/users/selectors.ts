...
import {
  UsersState, User, UIfiltersRoles, UIfiltersSearch,
} from './model';
...
export const getVisibleUsers = createSelector(
  ...
  (users, roles_filter, search_filter) => users.filter(
    (user) => user.role
                && roles_filter
...
export const isRequestInProgress = (state: UsersState, request_type: UsersActionType): boolean => state.inProgressRequests.includes(request_type);
