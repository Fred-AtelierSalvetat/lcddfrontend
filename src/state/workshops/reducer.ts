...
const workshops = (state: Workshop[] = [], action) => {
  switch (action.type) {
    ...
    case actionType.GO_LIVE_WORKSHOP:
      return state.map((workshop) => (workshop.id !== action.id ? workshop : { ...workshop, status: status.LIVE }));
    case actionType.END_LIVE_WORKSHOP:
      return state.map((workshop) => (workshop.id !== action.id ? workshop : { ...workshop, status: status.UNPUBLISHED }));
    case actionType.ARCHIVE_WORKSHOP:
      return state.map((workshop) => (workshop.id !== action.id ? workshop : { ...workshop, status: status.ARCHIVED }));
    case actionType.RESTORE_WORKSHOP:
      return state.map((workshop) => (workshop.id !== action.id ? workshop : { ...workshop, status: status.PUBLISHED }));
    default:
      return state;
  }
};
