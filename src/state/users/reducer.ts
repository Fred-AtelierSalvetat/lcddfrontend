...
const users = (state: User[] = [], action) => {
  switch (action.type) {
    ...
    case types.PROMOTE_USER_TO_ADMIN_SUCCESS:
      return state.map((user) => (user.user_id === action.user_id ? { ...user, role: userModel.ROLE_ADMIN } : user));

    case types.PROMOTE_USER_TO_SPEAKER_SUCCESS:
      return state.map((user) => (user.user_id === action.user_id ? { ...user, role: userModel.ROLE_SPEAKER_AWAITING_ANSWER } : user));
    case types.REVOKE_USER_ADMIN_RIGHT_SUCCESS:
    case types.VALIDATE_SPEAKER_SUCCESS:
      return state.map((user) => (user.user_id === action.user_id ? { ...user, role: userModel.ROLE_SPEAKER } : user));
    case types.ACTIVATE_USER_SUCCESS:
      return state.map((user) => (user.user_id === action.user_id ? { ...user, status: userStatus.ACTIVE } : user));
    case types.DEACTIVATE_USER_SUCCESS:
      return state.map((user) => (user.user_id === action.user_id ? { ...user, status: userStatus.INACTIVE } : user));
